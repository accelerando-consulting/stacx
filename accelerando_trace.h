...
int debug_aux_level = debug_level;
...
void __LEAF_DEBUG_PRINT__(const char *func,const char *file, int line, const char *leaf_name, int l, const char *fmt, ...)
{
  ...
  if (debug_aux && (l >= debug_aux_level)) {
    debug_aux->println(buf);
  }
...
#define LEAF_ENTER_AT(loc,l)  int enterlevel=l; unsigned long entertime=millis(); if (getDebugLevel()>=l) {__LEAF_DEBUG_AT__(loc, l,">%s", __func__)}
...
#define LEAF_ENTER_CSTRPAIR(l,s1,s2)  int enterlevel=l; unsigned long entertime=millis(); if (getDebugLevel()>=l) {__LEAF_DEBUG__(l,">%s(%s, %s)", __func__, (s1), (s2))}
...
#define LEAF_BOOLPAIR_RETURN(x,y)  LEAF_SLOW_CHECK;  __LEAF_DEBUG__(enterlevel,"<%s %s,%s", __func__, TRUTH(x),TRUTH(y)); return (x)
