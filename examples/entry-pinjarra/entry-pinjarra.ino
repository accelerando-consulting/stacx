...
#include "leaf_pinextender_74595.h"
...
Leaf *leaves[] = {
...
#if USE_PREFS
  (new FSLeaf("fs"))->setTrace(L_INFO),
  (new FSPreferencesLeaf("prefs"))->setTrace(L_INFO),
#endif
  new ShellLeaf("shell"),
  
  //(new IpEspLeaf("wifi","fs,prefs"))->setTrace(L_NOTICE),
  ...
  (new PinExtender74595Leaf("relays", 4, "~lock,~entrylight,~porchlight,~stairlight",
			    /*DS=*/D5, /*SHCP=*/D8, /*STCP=*/D7, /*~MR-*/-1,/*~OE=*/D6))->setTrace(L_INFO),
  (new LockLeaf("entry" ,      "relays", "lock", true, true))->setTrace(L_INFO), 
  new ContactLeaf("entry",    LEAF_PIN( A0 /* IN*/)),
  new ButtonLeaf("egress",    LEAF_PIN( D0 /* IN*/)),
  
  new ToneLeaf("spkr",        LEAF_PIN( D5 /*OUT*/)),
  (new LightLeaf("entry", "relays", "entrylight"))->setTrace(L_NOTICE),
  (new LightLeaf("porch", "relays", "porchlight"))->setTrace(L_NOTICE),
  (new LightLeaf("stair", "relays", "stairlight"))->setTrace(L_NOTICE),
  new MotionLeaf("porch",     LEAF_PIN( D3 /* IN*/)),
  new MotionLeaf("entry",     LEAF_PIN( D4 /* IN*/)),
  
  (new EntryAppLeaf("entry", "relays,light@entry,button@egress,motion@porch,motion@entry"))->setTrace(L_NOTICE),
