...
ifeq ($(MONITOR),miniterm)
	while : ; do if [ -e $(PORT) ] ; then $(MINITERM) --raw --rts $(MONITOR_RTS) --dtr $(MONITOR_DTR) $(MONITOR_ARGS) $(PORT) $(MONITOR_BAUD) ; echo "miniterm exit $$?" ; else printf "\rwait for modem `date +%T`" ; fi ; sleep 1 ; done || true
	stty sane
endif
ifeq ($(MONITOR),minitermonce)
	$(MINITERM) --raw --rts $(MONITOR_RTS) --dtr $(MONITOR_DTR) $(MONITOR_ARGS) $(PORT) $(MONITOR_BAUD) || true
	stty sane
endif
else
ifeq ($(MONITOR),cu)
...
ifeq ($(MONITOR),miniterm)
	ssh -t $(MONITORHOST) 'while true ; do if [ -e $(PROXYPORT) ] ; then $(MINITERM) --raw --rts $(MONITOR_RTS) --dtr $(MONITOR_DTR) $(MONITOR_ARGS) $(PROXYPORT) $(MONITOR_BAUD) ; else printf \"\\rwait for modem \`date +%T\`\" ; fi ; sleep 1 ; done' || true
endif
endif
