...
{
public:
  IpEspLeaf(String name) : AbstractIpLeaf(name) {
    LEAF_ENTER(L_INFO);
    LEAF_LEAVE;
  }
  // more public methods at end of file
...
public:
  virtual void setup()
  {
    ...
    _gotIpEventHandler = WiFi.onStationModeGotIP(
      [this](const WiFiEventStationModeGotIP& event){this->_wifi_connect_callback(event)}
      );
#else // ESP32
    WiFi.onEvent(
      [this](WiFiEvent_t event, WiFiEventInfo_t info) {this->_wifi_connect_callback(event,info)},
